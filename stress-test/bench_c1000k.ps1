...
# ============================================================
# Compares coronet (ST/chain/MT) vs ASIO (ST/MT) echo servers
# using redis-benchmark for load generation.
#
# Usage:
#   powershell -ExecutionPolicy Bypass -File bench_c1000k.ps1
#   powershell -ExecutionPolicy Bypass -File bench_c1000k.ps1 -Requests 200000 -Clients 200
# ============================================================
...
$BuildDir = $null
$candidates = @(
    (Join-Path $RepoRoot "buildmsvc\stress-test"),
    (Join-Path $RepoRoot "build\stress-test\Debug"),
    (Join-Path $RepoRoot "build\stress-test\Release"),
    (Join-Path $RepoRoot "build\stress-test")
)
foreach ($dir in $candidates) {
    if (Test-Path (Join-Path $dir "stress_driver.exe")) {
        $BuildDir = $dir
        break
    }
}
if (-not $BuildDir) {
    Write-Host "ERROR: Cannot find build directory with stress_driver.exe" -ForegroundColor Red
...
# Register cleanup to run on script exit (normal or error)
try {
    # Placeholder for trap registration
} catch {}
# PowerShell $PID tracking — use finally-equivalent via end block
...
# Suppress the built-in cleanup on exit — we handle it ourselves
$ranMain = $false
...
"@
    ...
    $r1a = Test-Server "coronet_ST"       (Join-Path $BuildDir $BinaryNames["coronet_ST"])    ($PortBase)
    $r1b = Test-Server "coronet_chain"    (Join-Path $BuildDir $BinaryNames["coronet_chain"]) ($PortBase + 1)
    $r1c = Test-Server "ASIO_ST"          (Join-Path $BuildDir $BinaryNames["ASIO_ST"])       ($PortBase + 2)
    ...
    $r2a = Test-Server "coronet_MT(6)"    (Join-Path $BuildDir $BinaryNames["coronet_MT"])    ($PortBase + 10) "6"
    $r2b = Test-Server "ASIO_MT(6)"       (Join-Path $BuildDir $BinaryNames["ASIO_MT"])       ($PortBase + 11) "6"
